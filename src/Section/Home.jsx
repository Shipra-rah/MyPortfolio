...
function Home() {
  const homeBtn = [{ name: "Get in Touch" }, { name: "See Project" }];
  return (
    <div className="w-full flex justify-center items-center flex-col gap-8 min-h-screen px-6 md:px-20 py-20">
      ...
      {/* Profile Content */}
      <div className="flex justify-center items-center gap-6 flex-col w-full max-w-4xl">
        <div className="text-4xl md:text-5xl lg:text-6xl flex justify-center items-center text-white tracking-wide uppercase font-bold">
          <Typing />
        </div>
        <div className="flex justify-center items-center w-full max-w-2xl px-4">
          <p className="text-base md:text-lg text-gray-300 text-center leading-relaxed font-light">
        ...
        <div className="flex justify-center items-center gap-6 md:gap-9 flex-wrap">
          {homeBtn.map((homeBtn, index) => (
            <HomeButton key={index} name={homeBtn.name} />
          ))}
        </div>
      </div>
